...
internal class WorkspaceServiceTest(
    ...
    fun `should fail to provide admin workspace access if user is not authenticated`() {
        assertThatThrownBy {
            runBlocking {
                workspaceService.getAccessibleWorkspace(preconditions.fryWorkspace.id!!, WorkspaceAccessMode.ADMIN)
            }
    ...
    fun `should fail to provide read-write workspace access if user is not authenticated`() {
        assertThatThrownBy {
            runBlocking {
                workspaceService.getAccessibleWorkspace(preconditions.fryWorkspace.id!!, WorkspaceAccessMode.READ_WRITE)
            }
        }.hasMessage("Authentication is not set")
    }
    ...
    fun `should fail to provide read-only workspace access if user is not authenticated`() {
        assertThatThrownBy {
            runBlocking {
                workspaceService.getAccessibleWorkspace(preconditions.fryWorkspace.id!!, WorkspaceAccessMode.READ_ONLY)
            }
        }.hasMessage("Authentication is not set")
    }
