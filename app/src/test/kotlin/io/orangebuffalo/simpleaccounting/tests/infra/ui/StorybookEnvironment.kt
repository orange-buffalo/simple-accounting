...
class StorybookEnvironment {
    ...
    private val localPlaywright = LocalPlaywrightApiProvider()
    private var useLocalPlaywright: Boolean = false

    private val contexts = ThreadLocal.withInitial {
        val storybookBaseUrl: String
        val playwrightApi: PlaywrightApi
        if (useLocalPlaywright) {
            storybookBaseUrl = if (TestEnvironment.config.screenshots.useCompliedStorybook)
                "http://localhost:${nginx.getMappedPort(80)}/"
            else
                "http://localhost:6006/"
            playwrightApi = localPlaywright.getOrCreatePlaywrightApiForCurrentThread()
        } else {
            storybookBaseUrl = if (TestEnvironment.config.screenshots.useCompliedStorybook)
                "http://storybook/"
            else "http://host.docker.internal:6006/"
            playwrightApi = TestEnvironment.browser.getPlaywrightApi()
        }
        val browser = playwrightApi.chromium()
        val context = browser.newContext(
            Browser.NewContextOptions()
                .setBaseURL(storybookBaseUrl)
    ...
    /**
     * Helper to debug screenshot tests. Should never be used to commit new screenshots,
     * as local browser may have different rendering comparing to container browser.
     */
    fun forceLocalPlaywright() {
        useLocalPlaywright = true
    }

    fun resetLocalPlaywright() {
        useLocalPlaywright = false
    }
}
