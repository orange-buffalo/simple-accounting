...
class UserProfileGoogleDriveDocumentStorageFullStackTest(
    ...
    fun `should disable Google Drive storage if was previously configured`(
        ...
        withClue("Google Drive should be turned on") {
            switch.shouldBeSwitchedOn()
            settings.shouldBeVisible()
        }
    ...
    @Test
    fun `should provide success status if configured before`(
        page: Page
    ) {
    ...
    private fun DocumentStorageConfig<GoogleDriveSettings>.assertAuthorizationRequiredStatus() {
        withClue("Should have authorization required status") {
            settings {
                shouldBeVisible()
    ...
    private fun PlatformUser.assertIntegrationFolderId(
        ...
        withClue("Should update the database state") {
