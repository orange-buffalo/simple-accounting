...
data class WorkspaceGqlDto(
    ...
    fun categories(env: DataFetchingEnvironment): CompletableFuture<List<CategoryGqlDto>> =
        env.getDataLoader<Long, List<CategoryGqlDto>>(CategoriesByWorkspaceIdDataLoader.NAME)!!
            .load(id)

    @GraphQLDescription("Expenses in this workspace.")
    fun expenses(env: DataFetchingEnvironment): CompletableFuture<List<ExpenseGqlDto>> =
        env.getDataLoader<Long, List<ExpenseGqlDto>>(ExpensesByWorkspaceIdDataLoader.NAME)!!
            .load(id)
}
...
data class ExpenseGqlDto(
    ...
    fun category(env: DataFetchingEnvironment): CompletableFuture<CategoryGqlDto?>? {
        ...
        return env.getDataLoader<Long, CategoryGqlDto?>(CategoryByIdDataLoader.NAME)!!
            .load(catId)
    }
}
