...
describe('translation files', () => {
  test('should provide same keys and shape for all languages', async () => {
    ...
    for (let i = 1; i < bundles.length; i += 1) {
      ...
      expect(otherBundleKeys, `Inconsistent keys for ${zeroBundle.language} / ${otherBundle.language}`)
        .to
        .eql(zeroBundleKeys);

      for (const bundleKey of zeroBundleKeys) {
        const zeroBundleValue = zeroBundleMessages[bundleKey];
        expect(zeroBundleValue)
          .to
          .be
        ...
        expect(otherBundleValue)
          .to
          .be
          .a('function', `All values in message bundle ${otherBundle.language} should be functions`)
          .to
          .have
          .property('length', argumentsLength, `${otherBundle.language} / ${bundleKey} should be consistent`);
      }
    }
  });
});
